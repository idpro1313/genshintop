...
### Приведение гайдов и персонажей к docs/PLAN.md: столпы, planTrack, мост по урону (1.12.2)
- **Что:** в **`info/README.md`** — матрица соответствия разделам **`docs/PLAN.md`**, полный список **`planTrack`** по slug, различие **`/guides/quests`** и **`/guides/quest-walkthroughs`**, блок про базу знаний через карточки и гайды. В **`docs/PLAN.md`** — раздел «Отражение в репозитории». Восстановлены **`docs/GUIDE_EDITORIAL.md`** и **`docs/CHARACTER_EDITORIAL.md`** (типы карточек why-pull / vs). Новый гайд **`info/guides/raschet-urona-mnozhiteli-i-kalkulyatory.md`**; правки **`stihii-i-reaktsii-baza.md`** (якоря по семи стихиям), **`fizicheskiy-i-elementalnyy-uron.md`**, **`storonnie-instrumenty-i-meta.md`**. Во всех **`info/guides/*.md`** добавлено **`planTrack`**. PHP: хабы **`/guides/game-basics`**, **`/guides/advanced-guides`**, **`/guides/quest-walkthroughs`** — **`lib/guide_hub_definitions.php`**, **`lib/GuideHub.php`**, **`lib/PageRenderer.php`** (каталог гайдов и перелинковка с каталогом персонажей), **`lib/SiteRoutes.php`**, **`lib/build-sitemap.php`**, **`lib/footer.php`**. **`docs/AGENTS.md`** — поле planTrack. **`grace/**`** и **`VERSION` → 1.12.2**. Точечно: **`info/characters/bennett.md`**, **`info/characters/why-pull-lyney.md`** — ссылка на новый гайд по урону.
- **Почему:** план работ — выровнять опорный корпус и навигацию сайта под **`docs/PLAN.md`** (документация, три столпа, пробел по расчёту урона).
- **Файлы:** перечисленные выше плюс массовое обновление **`info/guides/*.md`**, **`docs/HISTORY.md`**
- **Решение:** PATCH **1.12.2** — IA и контент без ломки старых тематических хабов; узкий **`/guides/quests`** сохранён.
