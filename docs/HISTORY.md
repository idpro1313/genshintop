...
### SEO-хабы и улучшение Docker-nginx (0.1.9)
- **Что:** подготовлены новые SEO-хабы персонажей и гайдов, расширена структура LootBar-раздела, добавлены страницы редакционной политики/раскрытия партнёрства, helper-компоненты и schema.org FAQ/HowTo; nginx для Docker перестал отдавать главную вместо несуществующих страниц и XML/TXT.
- **Почему:** фиксация текущего набора изменений перед публикацией и корректная обработка SSG-маршрутов/404 после деплоя.
- **Файлы:** `src/pages/characters/*.astro`, `src/pages/guides/*.astro`, `src/pages/lootbar/**`, `src/pages/404.astro`, `src/pages/editorial-policy.astro`, `src/pages/partnership-disclosure.astro`, `src/pages/contacts.astro`, `src/pages/content-updates.astro`, `src/components/*Hub*.astro`, `src/components/LootBar*.astro`, `src/content.config.ts`, `src/content/guides/*.md`, `src/layouts/BaseLayout.astro`, `src/lib/character-hub.ts`, `src/lib/guide-hub.ts`, `src/lib/partners.ts`, `src/lib/seo.ts`, `deploy/nginx-docker.conf`, `deploy/SEO-CHECKLIST.md`, `deploy/README.md`, `VERSION`, `package.json`, `package-lock.json`, `docs/AGENTS.md`, `grace/knowledge-graph/knowledge-graph.xml`, `grace/requirements/requirements.xml`, `grace/verification/verification-plan.xml`, `docs/HISTORY.md`
- **Решение:** версия поднята до `0.1.9`; lockfile синхронизирован с `VERSION`; проверка рантаймом/сборкой локально не запускалась по правилу проекта.
