...
- **M-CONTENT-V2** — параллельный каталог **`content/`** для новой редакции сайта по [`docs/PLAN.md`](PLAN.md). Зеркалит все разделы PLAN: characters/, weapons/, artifacts/, materials/, enemies/, guides/{basics,advanced,walkthroughs}/, tools/, world/{regions,lore,factions,npc}/, news/{events,announcements,banners,patches}/, community/. Стандарты — **`content/STYLE.md`**, шаблоны frontmatter — **`content/_templates/*.md`**. На момент 1.16.0 все рабочие разделы `content/` имеют `status: live` во frontmatter (шаблонные `stub` остаются только в `content/_templates/`): 115 characters, 57 weapons, 38 artifacts, 28 materials, 20 enemies, 37 guides, 24 factions, 20 NPC, 43 patches, 19 banners, 6 events, 5 announcements, 4 community, 4 tools. 8 персонажей расширены до full wiki (Albedo, Kuki, Alhaitham, Ayaka, Fischl, Sucrose, Beidou, Navia). Сайт пока **не** читает `content/` — `lib/ContentRepository.php` остаётся на `info/`. Переключение источника контента — отдельная задача Phase-7 step-7 после предмиграционной проверки ссылок.
