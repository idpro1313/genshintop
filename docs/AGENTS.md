...
Рантайм и репозиторий **без Node/npm**: только PHP, Markdown и статика. **`/sitemap.xml`** отдаётся PHP (`lib/SitemapBuild.php`): в карте есть **`<lastmod>`** по датам контента и служебным файлам; превью в CLI — `php bin/generate-sitemap.php`. Маршрут **`/rss.xml` не используется** (ответ 404).
...
- **M-PHP-SITE** — **`public/index.php`** (тонкая точка входа nginx), **`lib/`** (весь PHP приложения без подпапок: **`bootstrap.php`**, **`config.php`**, **`web_dispatch.php`**, классы, **`layout.php`**, **`header.php`**, **`footer.php`**, **`lootbar_banner.php`**, **`og-manifest.json`**, **`IndexNow.php`**, **`SitemapBuild.php`**), **`bin/indexnow-ping.php`** (IndexNow по URL из SitemapBuild), **`bin/generate-sitemap.php`** (stdout XML), **`public/css/site.css`** (тёмная тема и компоненты в духе [idpro1313/dandangers](https://github.com/idpro1313/dandangers) `modern-styles.css`: teal/violet, карточный prose; опционально light через `prefers-color-scheme`), **`public/og/`** (per-page PNG по `og-manifest`), фавиконки **`public/favicon.{ico,svg}`**, **`public/favicon-{16x16,32x32}.png`**, **`public/apple-touch-icon.png`** (он же fallback OG-картинки 180×180), **`public/icon-{192,512}.png`**, **`public/site.webmanifest`**, **`public/<INDEXNOW_KEY>.txt`** (верификация IndexNow), Docker/nginx. JSON-LD и мета через **`lib/Seo.php`** (включая `Seo::genshinVideoGameNode()` для `about` у Article-страниц и `Seo::ogImageDimensions()`), OG через **`OgManifest`**. Last-Modified/304 и 301 для trailing slash — в **`lib/Router.php`** (`Router::send` шлёт `Cache-Control: public, max-age=0, must-revalidate` и `Last-Modified` если страница передала `lastModifiedTs`; **`Router::sendSitemap`** — XML sitemap с `Cache-Control: public, max-age=3600` и условным 304). Mtime контентных страниц — **`ContentRepository::itemMtime()`** (max из frontmatter-дат и `filemtime`) / **`ContentRepository::latestMtime(callable)`**. Markdown-постпроцессор — **`ContentRepository::seoEnhanceContentHtml()`** (lazy `<img>`, `rel="nofollow noopener" target="_blank"` для внешних ссылок). Партнёрские ссылки — **`lib/Partners.php`**, LootBar — **`lib/LootbarConfig.php`**. Раздел **`/lootbar`** состоит из трёх страниц: хаб **`/lootbar`** (с якорными секциями `#promokod`, `#kristally`, `#welkin`, `#bezopasnost`), лендинг **`/lootbar/skidki-i-kupony`**, инструкция **`/lootbar/kak-popolnit-genshin-impact`**; плашка-баннер **`lib/lootbar_banner.php`** — единственная кликабельная ссылка на лендинг скидок. Каталог гайдов: поиск `?q=` и фильтры в **`lib/PageRenderer.php`** (`?q=` отдаёт `noindex, follow`); каталог персонажей: фильтры по стихии/оружию/редкости; generic content-роутер обслуживает остальные `content/`-разделы. `ContentRepository::markdownToHtml` после Parsedown пропускает HTML через `sanitizeContentLinks`: для относительных href срезается `.md`, ссылки на служебные пути (`_templates/`, `_by-`, `STYLE.md`, `README.md`, `info/`) разворачиваются в plain text без href, а лейблы вида `<code>foo.md</code>` чистятся от `.md` и обёртки `<code>`.
- С 1.19.1 URL контентных страниц строится по фактическому пути файла внутри `content/`, а не по сырой склейке `section + slug`; это устраняет двойные префиксы и коллизии `_index.md`. `Frontmatter` поддерживает стандартные indented lists и `sources.web`. Все статьи из `content/guides/**` рендерятся через `PageRenderer::guideArticle`, включая вложенные `/guides/basics/*`. Docker-образ копирует `VERSION` и `bin/`, поэтому футер и IndexNow CLI работают внутри контейнера.
- **M-CONTENT-GUIDE-REFACTOR** — legacy-корпус **`info/guides`**, **`info/characters`**; публичный рантайм его не читает, nginx legacy-redirect snippets в образ не подключаются.
- **M-CONTENT-V2** — основной каталог **`content/`** для сайта по редакционному плану. Зеркалит все разделы PLAN: characters/, weapons/, artifacts/, materials/, enemies/, guides/{basics,advanced,walkthroughs}/, tools/, world/{regions,lore,factions,npc}/, news/{events,announcements,banners,patches}/, community/. Стандарты — **`content/STYLE.md`**, шаблоны frontmatter — **`content/_templates/*.md`**. На момент 1.17.0 сайт переключён на чтение из `content/`. Старый `info/` остаётся только legacy-корпусом для сверки и отключён от публичного роутера. Начиная с 1.16.0 все рабочие разделы `content/` имеют `status: live` во frontmatter.
...
lib/
  ...
  *.php                 - Router, PageRenderer, Seo, контент, хабы, OG, IndexNow
...
public/
  ...
  <INDEXNOW_KEY>.txt    - верификация ключа IndexNow (имя файла = сам ключ)
docker/
  Dockerfile, docker-compose.yml, nginx-default.conf, supervisord.conf, docker-entrypoint.sh
  env.example (шаблон docker/.env, в т.ч. INDEXNOW_KEY), README.md — деплой Traefik/GHCR
