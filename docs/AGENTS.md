...
- **M-CONTENT-V2** — параллельный каталог **`content/`** для новой редакции сайта по [`docs/PLAN.md`](PLAN.md). Зеркалит все разделы PLAN: characters/, weapons/, artifacts/, materials/, enemies/, guides/{basics,advanced,walkthroughs}/, tools/, world/{regions,lore,factions,npc}/, news/{events,announcements,banners,patches}/, community/. Стандарты — **`content/STYLE.md`**, шаблоны frontmatter — **`content/_templates/*.md`**. На момент 1.13.0 содержит каркас + полные опорные материалы (5 walkthroughs, 4 PLAN-anchored basics, 7 advanced, 7 регионов, 6 лор-материалов, 12 фракций, ключевые tools/community/materials/enemies/weapons/artifacts/persons). Сайт пока **не** читает `content/` — `lib/ContentRepository.php` остаётся на `info/`. Переключение источника контента — отдельная задача после волны 2 (~180 carcards, оставшиеся basics, остальные оружия/артефакты/баннеры/патчи).
