...
8. В **sitemap.xml** у URL гайдов и персонажей присутствует **`lastmod`** (генерация при сборке образа / `scripts/build-sitemap.php`).
...
|---------------------------|------------------|
| `lootbar_banner_click` | Плашка — CTA «Получить скидку» (`templates/partials/lootbar_banner.php`) |
...
Подписка на клики: **`templates/layout.php`** (обработчик `[data-reach-goal]`).
