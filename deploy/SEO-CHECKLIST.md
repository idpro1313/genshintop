...
6. `https://ВАШ_ДОМЕН/rss.xml` — отдаётся **RSS** (`Content-Type` XML/RSS), в `<head>` страниц есть `<link rel="alternate" type="application/rss+xml" …>`.
7. `https://ВАШ_ДОМЕН/guides?q=баннер` — строка поиска на странице каталога подставляется и фильтрует карточки (для пользователя и соответствия SearchAction в JSON-LD).
8. В **sitemap-*.xml** у URL гайдов/персонажей присутствует **`lastmod`** (после релиза с генерацией из frontmatter/fs).
