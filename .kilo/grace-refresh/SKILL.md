...
Read `grace/knowledge-graph/knowledge-graph.xml` and, when present, `grace/verification/verification-plan.xml`. Identify:
...
- Missing verification entries - add or repair the matching `V-M-xxx` block in `grace/verification/verification-plan.xml`
...
Apply approved fixes to `grace/knowledge-graph/knowledge-graph.xml` and `grace/verification/verification-plan.xml` as needed. Update versions only after the selected refresh scope is reconciled.
