...
```
Knowledge Graph (grace/knowledge-graph/knowledge-graph.xml)
...
Verification Plan (grace/verification/verification-plan.xml)
...
`grace/knowledge-graph/knowledge-graph.xml` is the single map of the entire project. When you add a module — add it to the graph. When you add a dependency — add a CrossLink. The graph never drifts from reality.
...
```
grace/requirements/requirements.xml          — WHAT the user needs (use cases, AAG notation)
        |
grace/technology/technology.xml            — WHAT tools we use (runtime, language, versions)
        |
grace/plan/development-plan.xml      — HOW we structure it (modules, phases, contracts)
        |
grace/verification/verification-plan.xml     — HOW we prove it works (tests, traces, log markers)
        |
grace/knowledge-graph/knowledge-graph.xml       — MAP of everything (modules, dependencies, exports, verification refs)
        |
...
1. `$grace-init` — создаёт `grace/` (XML по этапам ЖЦ), `docs/AGENTS.md`, при отсутствии — `docs/HISTORY.md`, `grace/README.md`
2. Заполни `grace/requirements/requirements.xml` сценариями и требованиями
3. Заполни `grace/technology/technology.xml` решениями по стеку
