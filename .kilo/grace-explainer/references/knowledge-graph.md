...
The file `grace/knowledge-graph/knowledge-graph.xml` is the single source of truth for the project's module structure. It maps every module, its exports, its dependencies, and how modules connect to each other.
...
Modules may carry a `<verification-ref>` pointing to the matching `V-M-xxx` entry in `grace/verification/verification-plan.xml`.
