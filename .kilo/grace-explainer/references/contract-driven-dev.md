...
The contract is written before the code. It comes from the development plan (`grace/plan/development-plan.xml`), which was approved by the user during the `$grace-plan` phase.
...
```
Requirements (grace/requirements/requirements.xml)
  -> Architecture (grace/plan/development-plan.xml)
...
3. **Update knowledge graph** — if you add/remove modules, update `grace/knowledge-graph/knowledge-graph.xml`
4. **Update verification plan** — if you change tests, required markers, or verification commands, update `grace/verification/verification-plan.xml`
...
This XML contract is the blueprint for the MODULE_CONTRACT in the source file. The matching verification entry in `grace/verification/verification-plan.xml` is the blueprint for how the module proves that it still satisfies the contract.
