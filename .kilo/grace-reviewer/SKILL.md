...
- step status updates in `grace/plan/development-plan.xml`
...
- [ ] `grace/verification/verification-plan.xml` has the correct `V-M-xxx` entry or an intentional exception
...
- [ ] `grace/knowledge-graph/knowledge-graph.xml` matches the accepted deltas for the current scope
- [ ] verification delta proposals match actual tests, commands, and required markers
- [ ] `grace/verification/verification-plan.xml` matches the accepted deltas for the current scope
- [ ] `grace/plan/development-plan.xml` step or phase status updates match what was actually completed
