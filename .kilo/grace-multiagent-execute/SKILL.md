...
## Prerequisites
- `grace/plan/development-plan.xml` must exist with module contracts and implementation order
- `grace/knowledge-graph/knowledge-graph.xml` must exist
- `grace/verification/verification-plan.xml` should exist with module-local verification commands and gate expectations
...
- One controller owns shared artifacts: `grace/plan/development-plan.xml`, `grace/knowledge-graph/knowledge-graph.xml`, `grace/verification/verification-plan.xml`, phase status, and execution queue
...
Read `grace/plan/development-plan.xml`, `grace/knowledge-graph/knowledge-graph.xml`, and `grace/verification/verification-plan.xml` once per run, then build the controller view of the execution queue.
...
5. For each wave, prepare a compact **execution packet** for every module containing:
    ...
    - module contract excerpt from `grace/plan/development-plan.xml`
    - module graph entry excerpt from `grace/knowledge-graph/knowledge-graph.xml`
    - dependency contract summaries for every module in `DEPENDS`
    - verification excerpt from `grace/verification/verification-plan.xml`, including module-local commands, required scenarios, required log markers, and target test files
...
- **Controller**:
  - owns `grace/plan/development-plan.xml`
  - owns `grace/knowledge-graph/knowledge-graph.xml`
  - owns `grace/verification/verification-plan.xml`
...
2. Apply graph delta proposals once, centrally, to `grace/knowledge-graph/knowledge-graph.xml`
3. Apply verification delta proposals once, centrally, to `grace/verification/verification-plan.xml`
4. Update `grace/plan/development-plan.xml` step status once per wave
...
- Update `grace/knowledge-graph/knowledge-graph.xml`, `grace/verification/verification-plan.xml`, and `grace/plan/development-plan.xml` with wave results
- Commit with format:
  ```
