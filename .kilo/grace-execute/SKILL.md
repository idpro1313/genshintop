...
## Prerequisites
- `grace/plan/development-plan.xml` must exist with an ImplementationOrder section
- `grace/knowledge-graph/knowledge-graph.xml` must exist
- `grace/verification/verification-plan.xml` should exist and define module-level checks for the modules you plan to execute
...
Read `grace/plan/development-plan.xml`, `grace/knowledge-graph/knowledge-graph.xml`, and `grace/verification/verification-plan.xml`, then build the execution queue.
...
3. Build a controller-owned execution packet for each step containing:
   ...
   - verification excerpt from `grace/verification/verification-plan.xml`, including module-local commands, critical scenarios, required log markers, and test-file targets
...
1. update `grace/knowledge-graph/knowledge-graph.xml` from the accepted graph sync output or graph delta proposal
2. update `grace/verification/verification-plan.xml` from the accepted verification delta proposal
3. update step status in `grace/plan/development-plan.xml` if the step format supports explicit completion state
...
1. update `grace/plan/development-plan.xml`: set the `Phase-N` element's `status` attribute to `done`
2. run the phase-level verification commands or gates referenced in `grace/verification/verification-plan.xml`
...
- `grace/plan/development-plan.xml` and `grace/verification/verification-plan.xml` are shared sources of truth; never deviate from the contract or from required evidence silently
