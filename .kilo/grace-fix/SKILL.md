...
1. Read `grace/knowledge-graph/knowledge-graph.xml` for module overview
2. Read `grace/verification/verification-plan.xml` for relevant scenarios, test files, or log markers if available
...
- read the matching `V-M-xxx` entry in `grace/verification/verification-plan.xml`
...
4. If the fix changed tests, commands, or required markers — update `grace/verification/verification-plan.xml`
