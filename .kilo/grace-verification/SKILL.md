...
## Prerequisites
- `grace/plan/development-plan.xml` must exist with planned modules or module contracts
- `grace/verification/verification-plan.xml` should exist; if it does not, create it from the GRACE init template before proceeding
...
`grace/verification/verification-plan.xml` is the canonical place where this evidence model lives.
...
- `grace/requirements/requirements.xml`
- `grace/technology/technology.xml`
...
Turn these into a verification matrix before writing or revising tests. Keep the matrix synced into `grace/verification/verification-plan.xml`.
...
### Step 4: Build or Refresh `grace/verification/verification-plan.xml`
...
Make these levels explicit in `grace/verification/verification-plan.xml` so execution packets can reuse them.
...
- Keep `grace/verification/verification-plan.xml` synchronized with real test files and commands
...
1. an updated `grace/verification/verification-plan.xml`
